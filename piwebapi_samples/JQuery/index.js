   ...
   */
    ...
    $('#txtPIWebAPIUrl').val(configDefaults.PIWebAPIUrl)
    $('#txtAssetServer').val(configDefaults.AssetServer)
    $('#txtPIServer').val(configDefaults.PIServer)

    $('#txtPIWebAPIUser').val(configDefaults.Name)
    enableSubmit()
  })
 ...
 */
  function enableSubmit () {
    if ($('#txtPIWebAPIUrl').val() && ($('#selAuthType option:selected').text() === 'Kerberos' || ($('#txtPIWebAPIUser').val() && $('#txtPIWebAPIPassword').val())) && $('#txtAssetServer').val() && $('#txtPIServer').val()) {
  ...
  function RunSelected () {
    ...
    switch ($( '#selAction option:selected').text()) {
      ...
      case 'Reduce Payload with Selected Fields':
        readAttributeSelectedFields(PIWebAPIUrl, AssetServer, Name, Password, AuthType)
        break
      ...
      case 'Create Template':
        createTemplate(PIWebAPIUrl, AssetServer, PIServer, Name, Password, AuthType)
        break
